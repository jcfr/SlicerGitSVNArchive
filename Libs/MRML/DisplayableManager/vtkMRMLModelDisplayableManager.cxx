  ...
  for (i=0; i<ndnodes; i++)
    ...
    if (ait == this->Internal->DisplayedActors.end() )
      ...
      if (!prop)
        {
        prop = vtkActor::New();
//        std::cout << "DM:" << this << ", Associating actor:" << prop << " with "
...
void vtkMRMLModelDisplayableManager::UpdateModelHierarchyVisibility(vtkMRMLModelHierarchyNode* mhnode, int visibility )
{
  ...
  if (dnode)
    {
//    std::cout << "DM:UpdateModelHierarchyVisibility "
//              << mhnode->GetID() << " / " << mhnode->GetName() << std::endl;
    ...
    if (iter != this->Internal->DisplayedActors.end())
      {
//      std::cout << "DM:UpdateModelHierarchyDisplay Found DisplayedVisibility set to:"<< visibility << std::endl;
...
void vtkMRMLModelDisplayableManager::UpdateModelHierarchyDisplay(vtkMRMLDisplayableNode *model)
{
  if (model)
    ...
    if (mhnode)
      {
//      std::cout << "DM:UpdateModelHierarchyDisplay "
//                << mhnode->GetID() << " / " << mhnode->GetName() << std::endl;
      ...
      for (int i=0; i<ndnodes; i++)
        ...
        if (dnode)
          {
//          std::cout << "DM:UpdateModelHierarchyDisplay DNode "
//                    << dnode->GetID() << " / " << dnode->GetName() << std::endl;
          std::map<std::string, vtkProp3D *>::iterator iter = this->Internal->DisplayedActors.find(dnode->GetID());
          if (iter != this->Internal->DisplayedActors.end())
            {
//            std::cout << "DM:UpdateModelHierarchyDisplay Found displayed actor - HIDE" << std::endl;
            vtkProp3D *actor = iter->second;
