...
std::string ToString(POD_TYPE value)
{
...
//----------------------------------------------------------------------------
template<typename POD_TYPE>
bool CheckPOD(int line, const std::string& description,
              POD_TYPE current, POD_TYPE expected)
{
  if(current != expected)
    {
...
bool TestDefaultValues()
{
  ...
  if (!CheckPOD<vtkIdType>(__LINE__, "GetCount", spy->GetCount(), 0))
    {
    return false;
    }

  if (!CheckPOD<vtkIdType>(
  ...
  if (!CheckPOD<vtkEventSpy::EventPropertyRecorder*>(
...
bool TestEventRecording()
{
  ...
  if (!CheckPOD<vtkIdType>(__LINE__, "GetCount", spy->GetCount(), count))
    {
    return false;
    }
...
bool TestEventPropertyRecorderUsingDerivation()
{
  ...
  if (!CheckPOD<vtkIdType>(__LINE__, "GetCount", spy->GetCount(), 1))
    {
    return false;
    }
